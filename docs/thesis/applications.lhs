...
multiplicity of every element is or how the list is ordered. Other
...
adding the path constructor means we get |x == y| for every |x ~ y|,
but also paths constructed from these new paths using |trans| and
|sym|, possibly in combination with paths that already existed.

\subsection{Coherence issues}

One thing we glossed over is the question whether |Quotient A rel| is
actually an \hset, given the fact that |A| is an \hset. This need not be
the case, as is exhibited by the case where |A| is taken to be |top|
and |rel| is the trivial relation. The resulting quotient is
equivalent to the circle, which is not an \hset: the loop |sound tt tt
tt : [ tt ] == [ tt ]| is not equal to |refl : [ tt ] == [ tt ]|.

In order to get an \hset, we therefore need to take the \ntruncation{0}
...
additional paths of |settruncate A|. If |B| happens to be an \hset,
...
have that for every |x : settruncate A|, |B x| is an \hset, then we are
...
means that if |B| happens to be an \hset, we can appeal to \UIP and we
are done.
