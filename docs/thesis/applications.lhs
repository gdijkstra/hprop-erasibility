...
the approach to also work with non-isomorphic views. \todo{Note that
  this is an example of quotients}
...
\todo{Example: implement sets as lists} to encode our data in such a
way that certain operations on the data can be implemented more
efficiently. An example of this is implementing a dictionary with a
binary search tree: there are multiple binary search trees that
represent the same dictionary, \ie contain the same key-value
pairs. If we pass two different trees representing the same dictionary
\index{dictionary} to an operation, we want the operation to yield the
same results.
...
\todoi{Make this a bit more clear why this is the case.}
...
that in order to prove that two values |a| and |b| of type |Sigma (x :
A) . B x| are propositionally equal, we need to show its fields are
propositionally equal as well:
...
definitions. To show that |ListImplA| and |OtherImplA|, we need to
show using univalence that |List A == Other A|, so the beginning of
...
\todoi{Mention that univalence allows us to more succinctly specify
  things: show what this concretely means and how this is an
  improvement.}
