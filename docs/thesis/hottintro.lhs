...
circle (see also~\cref{fig:circle}):
...
The above is of course not (yet) valid Agda syntax. We can simulate
\hits by adding the extra path constructors (in this case |loop|) as
postulates. We can also hide the constructors in such a way, that we
can use them (indirectly) to construct terms of type |Circle|, but
without allowing pattern matching.
...
Instead of pattern matching, we specify how we can eliminate
inhabitants with the principle presented below. Roughly speaking we
need to ensure that all the points get mapped in such a way that all
the (extra) equalities are respected. In the case of the circle this
looks as follows:
